...
async function postReview(
  ...
  if (payload.requestReviewers.length) {
    await fetch(`https://api.github.com/repos/${repo}/pulls/${pr}/requested_reviewers`, {
      method: 'POST',
      headers: {
        authorization: `Bearer ${token}`,
        accept: 'application/vnd.github+json',
...
async function main(): Promise<void> {
  ...
  const verdict = await review({ intent: flag('intent') ?? '', diff }, graph, {});
  const payload = toGitHubReview(verdict, flag('team') ?? 'razorpay/design-system');
